...
NODE_MODULES_MASK_VOLUME="sheriff_empty_node_modules"
PROJECT_MOUNT="$(pwd):/data"
...
if ! docker image inspect "$IMAGE_NAME" &>/dev/null; then
  ...
  docker run -v "$PROJECT_MOUNT" -v "${NODE_MODULES_MASK_VOLUME}:/data/front-end/node_modules" ${IMAGE_NAME} --reset
else
...
# Back-end
docker run -v "$PROJECT_MOUNT" -v "${NODE_MODULES_MASK_VOLUME}:/data/front-end/node_modules" ${IMAGE_NAME} --test JAVA_HEXAGONAL --uri file:/data --component back-end
# Front-end
docker run -v "$PROJECT_MOUNT" -v "${NODE_MODULES_MASK_VOLUME}:/data/front-end/node_modules" ${IMAGE_NAME} --test TYPESCRIPT_HEXAGONAL --uri file:/data --component front-end
# Summary
docker run -v "$PROJECT_MOUNT" -v "${NODE_MODULES_MASK_VOLUME}:/data/front-end/node_modules" ${IMAGE_NAME} --summary
