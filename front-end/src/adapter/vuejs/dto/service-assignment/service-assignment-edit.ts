...
/**
 * ServiceAssignmentEdit
 * 
 * Data Transfer Object for editing an existing ServiceAssignment entity.
 * 
 * This DTO is designed to be used in Vue.js forms for updating service assignments.
 * It extends the functionality of ServiceAssignmentAdd by including the entity ID
 * and a method to create a DTO from an existing ServiceAssignment entity.
 * 
 * All attributes are public to allow direct binding with Vue.js form components.
 */
export class ServiceAssignmentEdit {
  /**
  ...
  /**
   * Create a new ServiceAssignmentEdit DTO.
   * 
  ...
  /**
   * Validate service assignment ID for form fields.
   * 
  ...
  /**
   * Generate a random ServiceAssignmentEdit instance for testing purposes.
   * 
   * @returns A new ServiceAssignmentEdit instance with random valid values
   */
  static random(): ServiceAssignmentEdit {
    ...
    return new ServiceAssignmentEdit(
      randomId,
      randomContainerId,
      randomFacilityId,
   ...
   * 
   * @param serviceAssignmentEdit ServiceAssignmentEdit DTO instance
   * @param container Container entity instance
   * @param facility Facility entity instance
   * @returns A new ServiceAssignment domain entity with the specified ID
   * @throws Error if any value object validation fails
   */
  static toServiceAssignment(
    serviceAssignmentEdit: ServiceAssignmentEdit,
    container: Container,
    facility: Facility
  ): ServiceAssignment {
    const id = new UllUUID(serviceAssignmentEdit.id);
    const quantityUnit = new QuantityUnit(serviceAssignmentEdit.wasteDemandQuantityUnit);
    const wasteDemand = new WasteDemand(
      serviceAssignmentEdit.wasteDemandValue,
      quantityUnit,
      serviceAssignmentEdit.wasteDemandTimeUnit as TimeUnit
    );
    const distance = Distance.fromMeters(serviceAssignmentEdit.distanceInMeters);
    const serviceTime = new ServiceTime(serviceAssignmentEdit.serviceTimeInMinutes);
    const transportCost = new TransportationVariableCost(
      serviceAssignmentEdit.transportCostPerKilometer,
      serviceAssignmentEdit.currencyCode
    );
  ...
  /**
   * Create a ServiceAssignmentEdit DTO from a ServiceAssignment domain entity.
   * 
   * @param serviceAssignment ServiceAssignment domain entity
   * @returns A new ServiceAssignmentEdit DTO with values extracted from the entity
   */
  static fromServiceAssignment(serviceAssignment: ServiceAssignment): ServiceAssignmentEdit {
    return new ServiceAssignmentEdit(
