    ...
    >
      <template #title-actions>
        ...
        <BulkImportFileButton
          :text="t('vehicle.list.importButton')"
          :tooltip="t('vehicle.list.importButton')"
          :importing="loading"
          @import="onImportVehiclesFile"
        />
      </template>
...
const onImportVehiclesFile = async (file: File) => {
  const result = await vehicleStore.importVehiclesFromFile(file);
  if (!result) {
    return;
  }
